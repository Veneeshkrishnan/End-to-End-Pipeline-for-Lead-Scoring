...
def get_models_prediction():
    ...
    try:
        ...
        pred_df.to_sql(name='predictions', con=connection,if_exists='replace',index=False)
        pred_df.to_csv(INFERENCE_PATH)
...
def prediction_ratio_check():
    ...
    try:
        ...
        f = open(FILE_PATH, "a")
        f.write(f"Prediction ratio for the date {time.ctime()} \n 1   {pct_ones}% \n 0   {pct_zeroes}%\n")
        f.close()
