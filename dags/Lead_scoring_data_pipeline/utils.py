...
def interactions_mapping():
    ...
    try:
        conn = sqlite3.connect(DB_PATH + DB_FILE_NAME)
        if not check_if_table_has_value(conn, 'model_input') or not check_if_table_has_value(conn, 'interactions_mapped'):
            ...
            trimmed_dataset.to_sql(name='model_input', con=conn, if_exists='replace', index=False) 
        else:
